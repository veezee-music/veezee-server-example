...
@section('body')
    ...
    <br>
    <form class="row" action="/{{ ADMIN_URL }}/playlists/edit-post/{{ $playlist['_id'] }}" method="post" enctype="multipart/form-data">
        <div class="col-lg-6">
            <div class="form-group">
                <label for="title">Title</label>
                <input type="text" name="title" id="title" value="{{ $playlist['title'] }}" class="form-control">
            </div>
        </div>
        <div class="col-lg-6 text-center">
            <div class="form-group">
                <label for="image">Artwork (square)</label>
                <input type="file" name="image" id="image" accept=".jpg,.png">
                @if(isset($playlist['image']))
                    <div>
                        <img id="artwork-img-preview" style="max-width: 10rem; max-height: 10rem; margin: 1rem;" src="/content/images/{{ $playlist['image'] }}">
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <input id="primaryColor" name="primaryColor" type="color" {{ isset($playlist['colors']['primaryColor']) ? "value=" . $playlist['colors']['primaryColor'] . "" : "" }}>
                            <br>
                            <label for="accentColor">Accent</label>
                            <input id="accentColor" name="accentColor" type="color" {{ isset($playlist['colors']['accentColor']) ? "value=" . $playlist['colors']['accentColor'] . "" : "" }}>
                        </div>
                    </div>
                @endif
            </div>
        ...
        <div class="col-lg-6 text-center">
            <div class="form-group">
                ...
                <input type="checkbox" name="headerActive" id="headerActive" {{ isset($playlist['header']) && $playlist['header']['active'] ? 'checked' : '' }}>
                <br><br>
                <label for="headerImage">Header image (rectangle)</label>
                <input type="file" name="headerImage" id="headerImage" accept=".jpg,.png">
                @if(isset($playlist['header']) && isset($playlist['header']['image']))
                    <div>
                        <img style="max-width: 20rem; max-height: 10rem; margin: 1rem;" src="/content/images/{{ $playlist['header']['image'] }}">
                    </div>
                @endif
            </div>
        </div>
...
@section('scripts')
    ...
    <script>
        ...
        function removeImage() {
            axios.post('/{{ ADMIN_URL }}/albums/remove-image-post/{{ $playlist['_id'] }}')
                .then(function (res) {
